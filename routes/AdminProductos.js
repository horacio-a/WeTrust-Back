...
router.get('/', async function (req, res, next) {
  ...
  for (let i = 0; i < producto.length; i++) {
    producto[i].img = cloudinary.url(producto[i].img,)
  }
...
router.post('/editar', async (req, res, next) => {
  try {
    ...
    console.log(error)
    res.redirect('/productos/editar/{req.body.id}'), {
      layout: 'admin/layout',
      error: true,
      message: 'No se modifico su producto vuelva a interntarlo'
    }
  }
})
